...
export function rotateFurniture(layout: OfficeLayout, uid: string, direction: 'cw' | 'ccw' = 'cw'): OfficeLayout {
  const item = layout.furniture.find((f) => f.uid === uid)
  if (!item) return layout
...
/** Get the furniture item at a specific tile position */
export function getFurnitureAtTile(
  ...
  for (const f of layout.furniture) {
    ...
    for (let dr = 0; dr < entry.footprintH; dr++) {
      for (let dc = 0; dc < entry.footprintW; dc++) {
        if (f.col + dc === col && f.row + dr === row) return f
      }
    }
  }
  return null
